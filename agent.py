...
■ 特徴テーマ統一ルール
  featuresの各項目は、その訴求軸のテーマに直接関連する内容のみで構成すること。
  【例】
  - コスト・価格訴求軸 → 価格帯・コスパ・予算対応に関する特徴に絞る
  - 課題解決訴求軸   → 発注プロセス・品質安定・サポート体制に関する特徴に絞る
  - 感情・ブランド訴求軸 → 差別化・ブランド価値・成果実感に関する特徴に絞る
  複数の訴求軸をまたいで同一テキストの特徴を繰り返さないこと。
  特に価格情報（例: 1本〇〇円〜）はコスト・比較系の軸以外のfeaturesには含めないこと。
