...
- バッジ・ラベル類：1個8文字以内・最大3個（提供された候補から構成に合うものを取捨選択。構成・サイズが合わない場合は0個でよい）
...
## テキストの表現技法（構成に合う場合のみ活用）

以下の手法はすべて「任意」。バナーの構成・トーン・訴求パターンに合うと判断した場合のみ使用すること。全バリエーションに必ず適用するのではなく、視覚的効果が明確に高まる場面でのみ選択する。

**① テキスト傾斜（ダイナミック演出）**
- 対象: ヘッドラインまたはアクセントコピー
- 効果: 動き・勢い・躍動感を演出する。縦長構成や余白が広い構成、感情訴求系パターンで特に有効
- 指定例: 「ヘッドラインを反時計回りに約-15°傾け、左下→右上方向に配置する」
- 注意: 傾斜は-30°以内。可読性が落ちるほど傾けないこと

**② アンダーライン強調**
- 対象: ヘッドライン内のキーワード1〜2語、またはサブコピーの一節
- 効果: 視線誘導・意味強調。特定の言葉を「読ませる」効果がある
- 指定例: 「『試着できる』の直下に白いアンダーライン（高さ4px・テキスト幅と同じ長さ）を引く」
- 注意: 下線は1箇所にとどめる。全体に引くと効果が薄れる

**③ 可変サイズ強調（キーワード拡大）**
- 対象: ヘッドラインのうち最も伝えたい1〜3文字
- 効果: キーワードが一目で飛び込んでくる。ファーストビューのインパクトが大幅に向上する
- 指定例: 「『試着』を120px・Boldで表示し、前後の文字（動画も、できる時代へ）は60pxで揃える。各テキストは同一行または近接した位置に配置し、視線が自然に流れるように構成する」
- 注意: サイズ変化は2段階まで。テキスト量が多い構成では使わない
