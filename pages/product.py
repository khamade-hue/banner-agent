...
st.markdown(
    ...
    'line-height:1.15;letter-spacing:-0.02em">商品登録・修正</h1>'
    '<p style="color:#86efac;margin:0;font-size:0.9rem;line-height:1.6;max-width:560px">'
    '訴求軸生成・バナー生成で使用する商品情報を登録・管理します'
    '</p></div>',
...
if _prod_mode == "新規登録":
    ...
    competitor_info = st.text_area(
        "競合商品情報（任意）",
        placeholder="競合他社名・製品名・強み・差別化ポイントなど。",
        height=90,
    )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        st.error(
            ...
            "**対処法:** [Supabase ダッシュボード](https://supabase.com/dashboard) でプロジェクトを確認し、"
            "一時停止中の場合は「Restore project」を実行してください。\n\n"
            f"エラー詳細: `{_e}`"
        )
        st.stop()
    ...
    if not products:
        st.markdown(
            ...
            '登録済みの商品がありません。「新規登録」タブから登録してください。</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
